...
export const sketch = (s) => {
    s.setup = () => {
        ...
        if (width > height) {
            ...
            cols_width = height / NUM_ROWS - 1 // -1 is a small offset so game doesn't takes the entire window
        }
        
        rows_height = cols_width;  //Grid should always be squares

        let board_w = Math.floor(cols_width * NUM_COLUMNS);
        let board_h = Math.floor(rows_height * NUM_ROWS);
        ...
        s.createCanvas(board_w + status_bar_w, board_h);
    ...
    s.windowResized = () => {
        ...
        //Mobile Ver
        if (height > width) cols_width = (width * 2 / 3) / NUM_COLUMNS; //Divide 2/3 of screen by number of cols
        else cols_width = height / NUM_ROWS - 1 // -1 is a small offset so game doesn't takes the entire window
